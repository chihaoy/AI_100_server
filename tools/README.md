...
| `moe_qwen3_cold_capacity.py`、`moe_qwen3_cold_capacity_report.py` | 捕获真实 layer 2 输入，固定权重/路由/重排与热组 C128，扫描冷组 C128–C2；结构一致性、逐轮输出、四卡 kernel/wait trace 与常量包审计；见 `9.17.2026/real_model/oracle_padding/cold_capacity_control/README.md` |
| `moe_qwen3_routing_retune.py`、`moe_qwen3_routing_retune_report.py` | 在零读取消与分块归约之后，对比 expert 行切片和七级整数前缀扫描，并重新扫描冷组 padding；无 profiling 延迟确认、逐轮 bit-exact 校验、四卡核放置与 P2P 流量；见 `9.17.2026/real_model/oracle_padding/routing_retune/README.md` |
