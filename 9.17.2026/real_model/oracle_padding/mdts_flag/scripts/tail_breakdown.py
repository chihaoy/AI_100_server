...
def group(r):
    ...
    if n in HOT or n in COLD:
        ...
        return 'stage other'
    if n.startswith('to_g_'): return 'combine: gather (token-owned)'
    if n.startswith('to_partials'): return 'combine: 8-row token reduce'
    if n.startswith(('to_sum', 'to_gm_')): return 'combine: mask/add (token-owned)'
    if n.startswith('to_'): return 'combine: index build (token-owned)'
    ...
    if n.startswith('final_tile_') or n == 'Einsum_4' or n.startswith('rs_'): return 'combine: card reduce'
