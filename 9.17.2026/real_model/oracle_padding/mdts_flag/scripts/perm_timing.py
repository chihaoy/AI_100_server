...
HOST = '/dev/shm/qwen3_cold_wentao_20260922/moe_single_host'; REF = np.fromfile(spec['ref_y'], np.float16).astype(np.float64)
def devices_ok():
...
for r in range(rounds):
    ...
    for case in cases:
        ...
        counts_ok = bool(np.array_equal(c, np.fromfile(case['counts'], np.int32))); rel = float(np.linalg.norm(y - REF) / np.linalg.norm(REF))
        ...
        print(f"round {r} {case['label']:>22s} median {m[1]} p10 {m[2]} p90 {m[3]}  counts exact {counts_ok}  y rel L2 vs sorted ref {rel:.2e}", flush=True)
